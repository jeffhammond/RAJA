...
struct scan_adapter {
  ...
  scan_adapter(scan_adapter& b, sycl::split)
...
struct scan_adapter_inclusive : scan_adapter<T, InIter, OutIter, Fn> {
  ...
  void operator()(const sycl::blocked_range<Index_type>& r, Tag)
  {
    T temp = this->agg;
...
concepts::enable_if<type_traits::is_sycl_policy<ExecPolicy>> inclusive_inplace(
    const ExecPolicy&,
    Iter begin,
    Iter end,
    BinFn f)
{
  ...
  sycl::parallel_scan(sycl::blocked_range<Index_type>{0,
...
concepts::enable_if<type_traits::is_sycl_policy<ExecPolicy>> exclusive_inplace(
    const ExecPolicy&,
    Iter begin,
    Iter end,
    BinFn f,
...
concepts::enable_if<type_traits::is_sycl_policy<ExecPolicy>> inclusive(
    const ExecPolicy&,
    const Iter begin,
    const Iter end,
    OutIter out,
...
template <typename ExecPolicy,
          ...
          typename BinFn,
          typename T>
concepts::enable_if<type_traits::is_sycl_policy<ExecPolicy>> exclusive(
    const ExecPolicy&,
    const Iter begin,
    const Iter end,
    OutIter out,
